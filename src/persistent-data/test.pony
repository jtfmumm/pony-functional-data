...
class iso _TestHAMTMap is UnitTest
  ...
  fun apply(h: TestHelper): TestResult ? =>
    ...
    h.expect_eq[Bool](H.isValue(m8.get("b"), 3), true)
    h.expect_eq[Bool](H.isValue(m8.get("d"), 4), true)
    h.expect_eq[Bool](H.isValue(m8.get("e"), 5), true)
    let m9 = m7.remove("e")
    h.expect_eq[Bool](H.isNone(m9.get("e")), true)
    h.expect_eq[Bool](H.isValue(m9.get("b"), 3), true)
    h.expect_eq[Bool](H.isValue(m9.get("d"), 4), true)
